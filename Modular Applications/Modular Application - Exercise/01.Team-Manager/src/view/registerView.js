...
export async function registerView(ctx){
  context = ctx;
  ctx.render(createRegisterTempalte())
}
...
async function onSubmit(e){
  ...
  if(password !== repass){
    return context.render(createRegisterTempalte('password don\'t match'))
  }
...
function createRegisterTempalte(err){
  return html`
    <section id="register">
      <article class="narrow">
          ...
          <form @submit=${onSubmit} id="register-form" class="main-form pad-large">
              ${err ? html`<div class="error">${err.msg}</div>` : ''}
